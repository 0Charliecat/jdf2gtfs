...
export default async function runtime(config: JDF2GTFS) {
	...
	for (let _ of _Spoje) {
		...
		//@ts-ignore
		let headsign = config.stopNames.get(_Zasspoje.findLast(z => z.lineNumber == _.lineNumber && z.tripNumber == _.tripNumber).stopId) // config.getStop(_Zasspoje.findLast(z => z.lineNumber == _.lineNumber && z.tripNumber == _.tripNumber).stopId)?.name
		...
		let computedTrip = new Trip({
			...
			headsign,
			direction: _linka?.isOneWay != "1" && ((+_.tripNumber % 2) === 0) ? TripDirection.Outbound : TripDirection.Inbound,
